...
https://atinjin.github.io/atinjin-pages/oratio/one-year

## 구성

- `oratio/one-year.html` — 페이지 전체 (HTML · CSS · JS가 한 파일에 담겨 있음). `/oratio/one-year` 경로로 서빙됩니다.
- `oratio/index.html` — 옛 `/oratio/` 주소를 `/oratio/one-year`로 넘겨주는 리다이렉트 페이지
- `oratio/og-card.png` — 링크 공유 시 뜨는 카드 이미지(1200×630)
- `.github/workflows/jekyll-gh-pages.yml` — `main` 브랜치에 push되면 GitHub Pages로 자동 배포
...
`oratio/one-year.html` 안의 `SONGS` 배열만 고치면 순위표가 바뀝니다.
`views` 칸에 조회수를, `date` 칸에 업로드 날짜를 적으면 카드에 함께 표시됩니다.
(예: `views:"1,234회", date:"2025. 8. 3"`)

엠블럼은 `oratio/emblem.png`, 채널 통계(특송·구독자 수)는 `one-year.html` 머리말의
