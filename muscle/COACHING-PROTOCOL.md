...
핵심 종목(v11, 축소 금지): 화 벤치프레스·버티컬 트랙션 / 수 스미스 스쿼트·스미스 RDL·머신 글루트 킥백 / 금 인클라인 덤벨 프레스·어시스트 풀업·랫풀다운 / 토 글루트·스미스 RDL·고블릿 스쿼트 (v17: 수요일 덤벨 RDL→스미스 RDL로 영구 전환, 하체A·B 동일 종목 / v20: 수요일 글루트→머신 글루트 킥백으로 영구 전환, 하체B는 그대로 글루트)
